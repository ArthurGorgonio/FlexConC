...
class CoFlexCon(BaseFlexConC):
    ...
    def fit(self, X, y):
        ...
        while not np.all(has_label) and (
            ...
            self.base_estimator_second_.fit(
                X2[safe_mask(X2, has_label2)], self.transduction_2[has_label2]
            )
            ...
            if self.n_iter_ > 1:
                pred_x1_it = self.storage_predict(
                ...
                dict_first = dict(sorted(dict_first.items(), key=lambda item: item[1]['confidence'], reverse=True))
                dict_first2 = self.storage_predict(
                    idx=np.nonzero(~has_label2)[0],
                    confidence=max_proba2,
                    classes=pred2,
                )
                dict_first2 = dict(sorted(dict_first2.items(), key=lambda item: item[1]['confidence'], reverse=True))
            ...
            try:
                if old_selected and old_selected2:
                    selected_full, new_pred = self.run_once(old_selected, selected, selected_full, pred1, self.transduction_)
                    self.base_estimator_select_.fit(X[selected_full], new_pred)
                    selected_full2, new_pred2 = self.run_once(old_selected2, selected2, selected_full2, pred2, self.transduction_2)
                    self.base_estimator_select_.fit(X[selected_full2], new_pred2)
                else:
                ...
                self.new_threshold(self.threshold2, local_acc2, init_acc)
            except ValueError:
    ...
    #TODO: MERGEAR ESSA BAGAÇA
        self.base_estimator_.fit(
            X[safe_mask(X, has_label)], self.transduction_[has_label]
        )
        self.base_estimator_second_.fit(
            X[safe_mask(X, has_label)], self.transduction_2[has_label]
        )
    ...
    #TODO: PAROU NO TRANSDUCTION
    def run_once(self, old_selected, selected, selected_full, pred, transduction):
